...
# C=.2
lookahead_list = [{'alpha':0.9, 'name': "sps_lookahead", 'c': .2,  'adapt_flag': 'mom1', 'momentum':0}] 
fixed_c = [{'name': "sps", 'c': .2,  'adapt_flag': 'mom1', 'momentum':0}]
neg_mom = [-.4, -.2]
pos_mom = [.2, .4,.6,.8]
for mom in pos_mom:
    fixed_c += [{'name': "sps", 'c': .2,  'adapt_flag': 'mom1', 'momentum':mom}]
    lookahead_list += [{'alpha':0.9, 'name': "sps_lookahead", 'c': .2,  'adapt_flag': 'mom1', 'momentum':mom}] 
...
EXP_GROUPS['fixed_c'] = get_benchmark(benchmarks=deep_dataset, opt_list=lookahead_list+ fixed_c)
