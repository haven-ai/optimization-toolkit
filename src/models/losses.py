...
def compute_cross_entropy(images, logits, masks, roi_masks=None):
    _, n_classes, _, _ = logits.shape
    if roi_masks is not None:
        masks[roi_masks==0] = 255   
    if n_classes == 1:
...
def compute_point_region_level():
    ...
    if ind.sum() > 0:
        loss += F.binary_cross_entropy_with_logits(logits[ind], 
                            points[ind].float().cuda(), 
                            reduction='mean')
def compute_point_level(images, logits, point_list, roi_masks=None):
    _, n_classes, _, _ = logits.shape
